...
class StorageObject(models.Model):
    ...
    def update_storage(self, force_digest=False): # TODO: PID update
        """
    ...
    def check_metadata(self):
        ...
        LOGGER.debug(u"\n_METADATA: {0}\n".format(_metadata))
        # change metaShareId
        _md = etree.ElementTree(etree.fromstring(_metadata))
        _iter = _md.getiterator()
        for i in _iter:
            if i.tag.endswith('metaShareId'):
                i.text = self.identifier
        uusjuur = _md.getroot()
        _metadata = b+etree.tostring(
            uusjuur).replace('ns0:','').replace(':ns0','')
