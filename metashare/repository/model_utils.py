...
# TODO: check if code below is needed (copied from EE metashare version)

def get_resource_languages(res_obj):
    """
    Returns a list of the language IDs 
    """
    result = []
    langs = []
    corpus_media = res_obj.resourceComponentType.as_subclass()
    ...
    if isinstance(corpus_media, corpusInfoType_model):
        ...
        for corpus_info in media_type.corpustextinfotype_model_set.all():
            ...
            for languages in languageinfo:
                ...
                result.append(languages.languageId)
        if media_type.corpusTextNgramInfo:
        ...
        if corpus_media.inputInfo:
            result.extend(corpus_media.inputInfo.languageId)
        if corpus_media.outputInfo:
            result.extend(corpus_media.outputInfo.languageId)
    return result, langs

def get_resource_languageNames(res_obj):
    """
    returns a list of language names but for specific media types (text and textNgram) 
    except when input or output of a toolService
    """
    result = []
    lang_infos = []
    corpus_media = res_obj.resourceComponentType.as_subclass()
    ...
    if isinstance(corpus_media, corpusInfoType_model):
        ...
        for corpus_info in media_type.corpustextinfotype_model_set.all():
            languageinfo = corpus_info.languageinfotype_model_set.all()
            lang_infos.append(languageinfo)
            for languages in languageinfo:
                result.append(languages.languageName)
        if media_type.corpusTextNgramInfo:
            languageinfo = media_type.corpusTextNgramInfo.languageinfotype_model_set.all()
            lang_infos.append(languageinfo)
            for languages in languageinfo:
        ...
        if corpus_media.inputInfo:
            lang_infos.extend(corpus_media.inputInfo)
            result.extend(corpus_media.inputInfo.languageName)
        if corpus_media.outputInfo:
            lang_infos.extend(corpus_media.outputInfo)
            result.extend(corpus_media.outputInfo.languageName)
    return result, lang_infos

# TODO: check if code above is needed (copied from EE metashare version)
